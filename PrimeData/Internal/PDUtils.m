...
+ (NSData *_Nullable)dataFromPlist:(nonnull id)plist
{
    ...
    NSData *data = [NSPropertyListSerialization dataWithPropertyList:plist
                                                             ...
                                                             options:0
                                                               error:&error];
    if (error) {
        PDLog(@"Unable to serialize data from plist object", error, plist);
    }
    return data;
}
    ...
    id plist = [NSPropertyListSerialization propertyListWithData:data
                                                         options:0
                                                          format:nil
                                                           error:&error];
    if (error) {
        PDLog(@"Unable to parse plist from data %@", error);
    }
    return plist;
}
...
+(id)traverseJSON:(id)object andReplaceWithFilters:(NSDictionary<NSString*, NSString*>*)patterns
{
    ...
    if ([object isKindOfClass:NSString.class]) {
        ...
        for (NSString* pattern in patterns) {
            ...
            if (matches > 0) {
                PDLog(@"%@ Redacted value from action: %@", self, pattern);
            }
        }
...
NSDictionary *getStaticContext(PDAnalyticsConfiguration *configuration, NSString *deviceToken)
{
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];

    dict[@"library"] = @{
        @"name" : @"analytics-ios",
        @"version" : [PDAnalytics version]
    };
...
#if TARGET_OS_IPHONE
NSDictionary *mobileSpecifications(PDAnalyticsConfiguration *configuration, NSString *deviceToken)
{
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
    ...
    /*#if !(TARGET_IPHONE_SIMULATOR)
        Class adClient = NSClassFromString(PDADClientClass);
        if (adClient) {
...
NSDictionary *desktopSpecifications(PDAnalyticsConfiguration *configuration, NSString *deviceToken)
{
...
NSDictionary *getLiveContext(PDReachability *reachability, NSDictionary *referrer, NSDictionary *traits)
{
...
@interface PDISO8601NanosecondDateFormatter: NSDateFormatter
@end

@implementation PDISO8601NanosecondDateFormatter
...
const NSInteger __PD_NANO_MAX_LENGTH = 9;
- (NSString * _Nonnull)stringFromDate:(NSDate *)date
{
    ...
    if (nanoSeconds.length > __PD_NANO_MAX_LENGTH) {
        nanoSeconds = [nanoSeconds substringToIndex:__PD_NANO_MAX_LENGTH];
    } else {
        nanoSeconds = [nanoSeconds stringByPaddingToLength:__PD_NANO_MAX_LENGTH withString:@"0" startingAtIndex:0];
    }
...
NSString *iso8601NanoFormattedString(NSDate *date)
{
    ...
    dispatch_once(&onceToken, ^{
        dateFormatter = [[PDISO8601NanosecondDateFormatter alloc] init];
    });
...
NSDictionary *PDCoerceDictionary(NSDictionary *dict)
{
...
{
    if ([obj conformsToProtocol:@protocol(PDSerializable)])
        return YES;
...
#else
            PDLog(@"key `%@` is a %@ and can't be serializaed for delivery.", key, className);
            // simply leave it out since we can't encode it anyway.
            continue;
#endif
        }
        
        if ([aValue conformsToProtocol:@protocol(PDSerializableDeepCopy)]) {
            theCopy = [aValue serializableDeepCopy:mutable];
        } else if ([aValue conformsToProtocol:@protocol(NSCopying)]) {
            theCopy = [aValue copy];
        } else if ([aValue conformsToProtocol:@protocol(PDSerializable)]) {
...
#else
            PDLog(@"found a %@ which can't be serializaed for delivery.", className);
            // simply leave it out since we can't encode it anyway.
            continue;
#endif
        }
