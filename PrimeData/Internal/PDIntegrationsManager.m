...
NSString *PDAnalyticsIntegrationDidStart = @"io.segment.analytics.integration.did.start";
NSString *const PDAnonymousIdKey = @"PDAnonymousId";
NSString *const kPDAnonymousIdFilename = @"segment.anonymousId";
NSString *const kPDCachedSettingsFilename = @"analytics.settings.v2.plist";


@interface PDIdentifyPayload (AnonymousId)
@property (nonatomic, readwrite, nullable) NSString *anonymousId;
@end


@interface PDPayload (Options)
@property (readonly) NSDictionary *options;
@end
@implementation PDPayload (Options)
...
@interface PDAnalyticsConfiguration (Private)
@property (nonatomic, strong) NSArray *factories;
@end


@interface PDIntegrationsManager ()

@property (nonatomic, strong) PDAnalytics *analytics;
@property (nonatomic, strong) NSDictionary *cachedSettings;
@property (nonatomic, strong) PDAnalyticsConfiguration *configuration;
...
@property (nullable, nonatomic, strong, readonly) PDAnalyticsConfiguration *oneTimeConfiguration;
@end


@implementation PDIntegrationsManager
...
{
    PDAnalyticsConfiguration *configuration = analytics.oneTimeConfiguration;
    ...
    if (self = [super init]) {
        ...
        #if TARGET_OS_TV
            self.fileStorage = [[PDFileStorage alloc] initWithFolder:[PDFileStorage cachesDirectoryURL] crypto:configuration.crypto];
        #else
            self.fileStorage = [[PDFileStorage alloc] initWithFolder:[PDFileStorage applicationSupportDirectoryURL] crypto:configuration.crypto];
        #endif

        self.cachedAnonymousId = [self loadOrGenerateAnonymousID:NO];
        NSMutableArray *factories = [[configuration factories] mutableCopy];
        [factories addObject:[[PDPrimeDataIntegrationFactory alloc] initWithHTTPClient:self.httpClient fileStorage:self.fileStorage userDefaultsStorage:self.userDefaultsStorage]];
...
{
    [PDState sharedInstance].userInfo.anonymousId = cachedAnonymousId;
}

- (NSString *)cachedAnonymousId
{
    NSString *value = [PDState sharedInstance].userInfo.anonymousId;
    return value;
}
...
{
    PDLog(@"Application state change notification: %@", notificationName);
...
{
#if TARGET_OS_TV
    NSString *anonymousId = [self.userDefaultsStorage stringForKey:PDAnonymousIdKey];
#else
    NSString *anonymousId = [self.fileStorage stringForKey:kPDAnonymousIdFilename];
#endif
    ...
    if (!anonymousId || reset) {
        ...
        PDLog(@"New anonymousId: %@", anonymousId);
#if TARGET_OS_TV
        [self.userDefaultsStorage setString:anonymousId forKey:PDAnonymousIdKey];
#else
        [self.fileStorage setString:anonymousId forKey:kPDAnonymousIdFilename];
#endif
    }
...
- (NSDictionary *)cachedSettings
{
    if (!_cachedSettings) {
#if TARGET_OS_TV
        _cachedSettings = [self.userDefaultsStorage dictionaryForKey:kPDCachedSettingsFilename] ?: @{};
#else
        _cachedSettings = [self.fileStorage dictionaryForKey:kPDCachedSettingsFilename] ?: @{};
#endif
    }
...
#if TARGET_OS_TV
    [self.userDefaultsStorage setDictionary:_cachedSettings forKey:kPDCachedSettingsFilename];
#else
    [self.fileStorage setDictionary:_cachedSettings forKey:kPDCachedSettingsFilename];
#endif

    [self updateIntegrationsWithSettings:settings[@"integrations"]];
}

- (nonnull NSArray<id<PDMiddleware>> *)middlewareForIntegrationKey:(NSString *)key
{
    NSMutableArray *result = [[NSMutableArray alloc] init];
    for (PDDestinationMiddleware *container in self.configuration.destinationMiddleware) {
...
- (void)updateIntegrationsWithSettings:(NSDictionary *)projectSettings
{
    seg_dispatch_specific_sync(_serialQueue, ^{
        ...
        for (id<PDIntegrationFactory> factory in self.factories) {
            ...
            if (integrationSettings || [key hasPrefix:@"webhook_"]) {
                id<PDIntegration> integration = [factory createWithSettings:integrationSettings forAnalytics:self.analytics];
                if (integration != nil) {
                    self.integrations[key] = integration;
                    self.registeredIntegrations[key] = @NO;
                    
                    // setup integration middleware
                    NSArray<id<PDMiddleware>> *middleware = [self middlewareForIntegrationKey:key];
                    self.integrationMiddleware[key] = [[PDMiddlewareRunner alloc] initWithMiddleware:middleware];
                }
                [[NSNotificationCenter defaultCenter] postNotificationName:PDAnalyticsIntegrationDidStart object:key userInfo:nil];
            } else {
                PDLog(@"No settings for %@. Skipping.", key);
            }
        }
...
- (void)refreshSettings
{
    [self setCachedSettings:@{
                               @"integrations" : @{
                                   @"PrimeData.io" : @{@"apiKey" : self.configuration.writeKey},
                               },
                               @"plan" : @{@"track" : @{}}
                           }];
...
//                        newSettings[@"integrations"][@"PrimeData.io"][@"apiKey"] = self.configuration.writeKey;
//                        [self setCachedSettings:newSettings];
//                        // don't configure edge functions here.  it'll do the right thing on it's own.
//                    } else {
//                        // If settings request fail, fall back to using just PrimeData integration.
...
+ (BOOL)isIntegration:(NSString *)key enabledInOptions:(NSDictionary *)options
{
    ...
    if ([@"PrimeData.io" isEqualToString:key]) {
        return YES;
    }
    if (options[key]) {
...
+ (BOOL)isTrackEvent:(NSString *)event enabledForIntegration:(NSString *)key inPlan:(NSDictionary *)plan
{
    ...
    if ([key isEqualToString:@"PrimeData.io"]) {
        return YES;
    }
...
{
    [self.integrations enumerateKeysAndObjectsUsingBlock:^(NSString *key, id<PDIntegration> integration, BOOL *stop) {
 ...
 */
- (PDEventType)eventTypeFromSelector:(SEL)selector
{
    ...
    if ([selectorString hasPrefix:@"identify"]) {
        ...
        result = PDEventTypeScreen;
    } else if ([selectorString hasPrefix:@"group"]) {
        result = PDEventTypeGroup;
    } else if ([selectorString hasPrefix:@"alias"]) {
        result = PDEventTypeAlias;
    } else if ([selectorString hasPrefix:@"reset"]) {
        result = PDEventTypeReset;
    } else if ([selectorString hasPrefix:@"flush"]) {
        result = PDEventTypeFlush;
    } else if ([selectorString hasPrefix:@"receivedRemoteNotification"]) {
        result = PDEventTypeReceivedRemoteNotification;
    } else if ([selectorString hasPrefix:@"failedToRegisterForRemoteNotificationsWithError"]) {
        result = PDEventTypeFailedToRegisterForRemoteNotifications;
    } else if ([selectorString hasPrefix:@"registeredForRemoteNotificationsWithDeviceToken"]) {
        result = PDEventTypeRegisteredForRemoteNotifications;
    } else if ([selectorString hasPrefix:@"handleActionWithIdentifier"]) {
        result = PDEventTypeHandleActionWithForRemoteNotification;
    } else if ([selectorString hasPrefix:@"continueUserActivity"]) {
        result = PDEventTypeContinueUserActivity;
    } else if ([selectorString hasPrefix:@"openURL"]) {
        result = PDEventTypeOpenURL;
    } else if ([selectorString hasPrefix:@"application"]) {
        result = PDEventTypeApplicationLifecycle;
    }

    return result;
}

- (void)invokeIntegration:(id<PDIntegration>)integration key:(NSString *)key selector:(SEL)selector arguments:(NSArray *)arguments options:(NSDictionary *)options
{
    if (![integration respondsToSelector:selector]) {
        PDLog(@"Not sending call to %@ because it doesn't respond to %@.", key, NSStringFromSelector(selector));
        return;
    }

    if (![[self class] isIntegration:key enabledInOptions:options[@"integrations"]]) {
        PDLog(@"Not sending call to %@ because it is disabled in options.", key);
        return;
    }
    
    PDEventType eventType = [self eventTypeFromSelector:selector];
    if (eventType == PDEventTypeTrack) {
        PDTrackPayload *eventPayload = arguments[0];
        BOOL enabled = [[self class] isTrackEvent:eventPayload.event enabledForIntegration:key inPlan:self.cachedSettings[@"plan"]];
        if (!enabled) {
            PDLog(@"Not sending call to %@ because it is disabled in plan.", key);
            return;
        }
    }
    ...
    if (eventType != PDEventTypeUndefined) {
        PDMiddlewareRunner *runner = self.integrationMiddleware[key];
        if (runner.middlewares.count > 0) {
            PDPayload *payload = nil;
            // things like flush have no args.
            if (arguments.count > 0) {
                payload = arguments[0];
            }
            PDContext *context = [[[PDContext alloc] initWithAnalytics:self.analytics] modify:^(id<PDMutableContext> _Nonnull ctx) {
    ...
    PDLog(@"Running: %@ with arguments %@ on integration: %@", NSStringFromSelector(selector), newArguments, key);
...
{
    struct objc_method_description description = protocol_getMethodDescription(@protocol(PDIntegration), selector, NO, YES);
...
@implementation PDIntegrationsManager (PDMiddleware)
...
- (void)context:(PDContext *)context next:(void (^_Nonnull)(PDContext *_Nullable))next
{
    switch (context.eventType) {
        ...
        case PDEventTypeTrack: {
            PDTrackPayload *p = (PDTrackPayload *)context.payload;
            [self track:p];
            break;
        }
        case PDEventTypeScreen: {
            PDScreenPayload *p = (PDScreenPayload *)context.payload;
            [self screen:p];
            break;
        }
        case PDEventTypeGroup: {
            PDGroupPayload *p = (PDGroupPayload *)context.payload;
            [self group:p];
            break;
        }
        case PDEventTypeAlias: {
            PDAliasPayload *p = (PDAliasPayload *)context.payload;
            [self alias:p];
            break;
        }
        case PDEventTypeReset:
            [self reset];
            break;
        case PDEventTypeFlush:
            [self flush];
            break;
        case PDEventTypeReceivedRemoteNotification:
            [self receivedRemoteNotification:
                      [(PDRemoteNotificationPayload *)context.payload userInfo]];
            break;
        case PDEventTypeFailedToRegisterForRemoteNotifications:
            [self failedToRegisterForRemoteNotificationsWithError:
                      [(PDRemoteNotificationPayload *)context.payload error]];
            break;
        case PDEventTypeRegisteredForRemoteNotifications:
            [self registeredForRemoteNotificationsWithDeviceToken:
                      [(PDRemoteNotificationPayload *)context.payload deviceToken]];
            break;
        case PDEventTypeHandleActionWithForRemoteNotification: {
            PDRemoteNotificationPayload *payload = (PDRemoteNotificationPayload *)context.payload;
            [self handleActionWithIdentifier:payload.actionIdentifier
                       forRemoteNotification:payload.userInfo];
            break;
        }
        case PDEventTypeContinueUserActivity:
            [self continueUserActivity:
                      [(PDContinueUserActivityPayload *)context.payload activity]];
            break;
        case PDEventTypeOpenURL: {
            PDOpenURLPayload *payload = (PDOpenURLPayload *)context.payload;
            [self openURL:payload.url options:payload.options];
            break;
        }
        case PDEventTypeApplicationLifecycle:
            [self handleAppStateNotification:
                      [(PDApplicationLifecyclePayload *)context.payload notificationName]];
            break;
        default:
        case PDEventTypeUndefined:
            NSAssert(NO, @"Received context with undefined event type %@", context);
            PDLog(@"[ERROR]: Received context with undefined event type %@", context);
            break;
    }
    next(context);
}
