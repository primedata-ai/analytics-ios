...
NSString *const PDPrimeDataDidSendRequestNotification = @"PrimeDataDidSendRequest";
NSString *const PDPrimeDataRequestDidSucceedNotification = @"PrimeDataRequestDidSucceed";
NSString *const PDPrimeDataRequestDidFailNotification = @"PrimeDataRequestDidFail";
...
@interface PDPrimeDataIntegration ()
...
@implementation PDPrimeDataIntegration
...
- (void)beginBackgroundTask
{
    ...
    seg_dispatch_specific_sync(_backgroundTaskQueue, ^{
        ...
        if (application && [application respondsToSelector:@selector(seg_beginBackgroundTaskWithName:expirationHandler:)]) {
            self.flushTaskID = [application seg_beginBackgroundTaskWithName:@"PrimeDataio.Flush"
                                                          expirationHandler:^{
                                                              [self endBackgroundTask];
                                                          }];
        }
...
//        }
        ...
        if (self.configuration.experimental.rawPrimeDataModificationBlock != nil) {
            NSDictionary *tempPayload = self.configuration.experimental.rawPrimeDataModificationBlock(queuePayload);
            if (tempPayload == nil) {
                PDLog(@"rawPrimeDataModificationBlock cannot be used to drop events!");
            } else {
...
- (void)sendData:(NSArray *)batch
{
    ...
    self.batchRequest = [self.httpClient upload_track_events:payload forWriteKey:self.configuration.writeKey completionHandler:^(BOOL retry) {
        void (^completion)(void) = ^{
            ...
            [self persistQueue];
            [self notifyForName:PDPrimeDataRequestDidSucceedNotification userInfo:batch];
            self.batchRequest = nil;
            [self endBackgroundTask];
        };
    ...
    [self notifyForName:PDPrimeDataDidSendRequestNotification userInfo:batch];
}
