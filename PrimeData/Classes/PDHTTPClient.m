...
- (instancetype)initWithRequestFactory:(PDRequestFactory)requestFactory
{
    if (self = [self init]) {
...
//    batch = body_track;
    ...
    //    batch = PDCoerceDictionary(batch);
    NSURLSession *session = [self sessionForWriteKey:writeKey];

    NSURL *url = [PDMENT_API_BASE URLByAppendingPathComponent:@"smile"];
...
//    if (error || exception) {
//        PDLog(@"Error serializing JSON for batch upload %@", error);
//        completionHandler(NO); // Don't retry this batch.
//        return nil;
//    }
//    if (payload.length >= kMaxBatchSize) {
//        PDLog(@"Payload exceeded the limit of %luKB per batch", kMaxBatchSize / 1000);
...
- (NSURLSessionDataTask *)settingsForWriteKey:(NSString *)writeKey completionHandler:(void (^)(BOOL success, JSON_DICT _Nullable settings))completionHandler
{
    ...
    NSURL *url = [PDMENT_API_BASE URLByAppendingPathComponent:[NSString stringWithFormat:@"/projects/%@/settings", writeKey]];
    NSMutableURLRequest *request = self.requestFactory(url);
