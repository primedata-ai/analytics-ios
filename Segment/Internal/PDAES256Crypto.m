...
{
    NSData *iv = [PDAES256Crypto randomDataOfLength:kAlgorithmIVSize];
    NSData *salt = [PDAES256Crypto randomDataOfLength:kPBKDFSaltSize];
...
- (NSData *)encrypt:(NSData *)data
{
    ...
    if (result == kCCSuccess) {
        ...
        NSError *error = [NSError errorWithDomain:kRNCryptManagerErrorDomain
                                             code:result
                                         userInfo:nil];
        PDLog(@"Unable to encrypt data", error);
        return nil;
    }
    return cipherData;
}
...
- (NSData *)decrypt:(NSData *)data
{
    ...
    if (result == kCCSuccess) {
        ...
        NSError *error = [NSError errorWithDomain:kRNCryptManagerErrorDomain
                                             code:result
                                         userInfo:nil];
        PDLog(@"Unable to decrypt data", error);
        return nil;
    }
...
+ (NSData *)randomDataOfLength:(size_t)length
{
    ...
    int result = SecRandomCopyBytes(kSecRandomDefault,
                                    ...
                                    data.mutableBytes);
    if (result != kCCSuccess) {
        PDLog(@"Unable to generate random bytes: %d", result);
    }
                         ...
                         salt:(NSData *)salt
{
    ...
    if (result != kCCSuccess) {
        PDLog(@"Unable to create AES key for password: %d", result);
    }
