...
@implementation PDUserDefaultsStorage

- (instancetype)initWithDefaults:(NSUserDefaults *)defaults namespacePrefix:(NSString *)namespacePrefix crypto:(id<PDCrypto>)crypto
{
    if (self = [super init]) {
        _defaults = defaults;
...
- (id _Nullable)plistForKey:(NSString *)key
{
    ...
    return data ? [PDUtils plistFromData:data] : nil;
}

- (void)setPlist:(id _Nonnull)plist forKey:(NSString *)key
{
    NSData *data = [PDUtils dataFromPlist:plist];
    if (data) {
