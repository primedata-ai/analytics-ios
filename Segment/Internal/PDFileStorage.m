...
@implementation PDFileStorage

- (instancetype)initWithFolder:(NSURL *)folderURL crypto:(id<PDCrypto>)crypto
{
    if (self = [super init]) {
...
- (void)removeKey:(NSString *)key
{
    ...
    if (![[NSFileManager defaultManager] removeItemAtURL:url error:&error]) {
        PDLog(@"Unable to remove key %@ - error removing file at path %@", key, url);
    }
}
...
- (void)resetAll
{
    ...
    if (![[NSFileManager defaultManager] removeItemAtURL:self.folderURL error:&error]) {
        PDLog(@"ERROR: Unable to reset file storage. Path cannot be removed - %@", self.folderURL.path);
    }
...
- (void)setData:(NSData *)data forKey:(NSString *)key
{
    ...
    if (![url setResourceValue:@YES
                        forKey:NSURLIsExcludedFromBackupKey
                         error:&error]) {
        PDLog(@"Error excluding %@ from backup %@", [url lastPathComponent], error);
    }
}
...
- (NSData *)dataForKey:(NSString *)key
{
    ...
    if (!data) {
        PDLog(@"WARNING: No data file for key %@", key);
        return nil;
    }
    if (self.crypto) {
    ...
    if (data) {
        ...
        if (needsConversion) {
            @try {
                ...
                PDLog(@"Unable to convert data from plist object to json; Exception: %@, data: %@", e, data);
...
- (NSData *_Nullable)dataFromJSON:(id)json
{
    ...
    if (error) {
        PDLog(@"Unable to serialize data from json object; %@, %@", error, json);
    }
    return data;
}
    ...
    if (error) {
        ...
        if (result != nil) {
            *needsConversion = YES;
            ...
            PDLog(@"Unable to parse json from data %@", error);
        }
    }
    return result;
}
...
- (NSData *_Nullable)dataFromPlist:(nonnull id)plist
{
    ...
    @try {
        ...
        PDLog(@"Unable to serialize data from plist object; Exception: %@, plist: %@", e, plist);
    } @finally {
        if (error) {
            PDLog(@"Unable to serialize data from plist object; Error: %@, plist: %@", error, plist);
        }
    }
    return data;
}
