XC_ARGS := -project PrimeData.xcodeproj GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=YES
...
XC_BUILD_ARGS := -scheme PrimeData ONLY_ACTIVE_ARCH=NO
...
archive: carthage
	carthage archive PrimeData

clean-ios:
	set -o pipefail && xcodebuild $(IOS_XCARGS) -scheme PrimeData clean | xcpretty

clean-tvos:
	set -o pipefail && xcodebuild $(TVOS_XCARGS) -scheme PrimeData clean | xcpretty

clean-macos:
	set -o pipefail && xcodebuild $(MACOS_XCARGS) -scheme PrimeData clean | xcpretty
...
test-ios:
	@set -o pipefail && xcodebuild test $(IOS_XCARGS) -scheme PrimeDataTests $(XC_TEST_ARGS) | xcpretty --report junit

test-tvos:
	@set -o pipefail && xcodebuild test $(TVOS_XCARGS) -scheme PrimeDataTests $(XC_TEST_ARGS) | xcpretty --report junit

test-macos:
	@set -o pipefail && xcodebuild test $(MACOS_XCARGS) -scheme PrimeDataTests $(XC_TEST_ARGS) | xcpretty --report junit

test: test-ios test-tvos test-macos

xctest:
	xctool $(IOS_XCARGS) -scheme PrimeDataTests $(XC_TEST_ARGS) run-tests -sdk iphonesimulator
