...
class PassthroughMiddleware: Middleware {
    ...
    func context(_ context: Context, next: @escaping PDMiddlewareNext) {
        lastContext = context;
        next(context)
    }
}
...
extension IntegrationsManager {
    ...
    func test_segmentIntegration() -> PrimeDataIntegration? {
        return self.test_integrations()?["PrimeData.io"] as? PrimeDataIntegration
    }
...
extension PrimeDataIntegration {
...
typealias AndPrimeDataWriteKeyMethod = (String) -> LSStubRequestDSL

extension LSStubRequestDSL {
    var withWriteKey: AndPrimeDataWriteKeyMethod {
        return { writeKey in
